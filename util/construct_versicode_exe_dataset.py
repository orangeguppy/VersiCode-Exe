...
def parse_args() -> argparse.Namespace:
    ...
    # Root directory containing the original VersiCode benchmark JSON files
    parser.add_argument("--original-root", type=Path, default=DEFAULT_ORIGINAL_ROOT)

    # Root directory containing validation result JSON files
    parser.add_argument("--validation-root", type=Path, default=DEFAULT_VALIDATION_ROOT)

    # Root directory where constructed VersiCode-Exe JSON files will be written
    parser.add_argument("--output-root", type=Path, default=DEFAULT_OUTPUT_ROOT)

    # Option to overwrite existing output files
    parser.add_argument("--overwrite", action="store_true")
    parser.add_argument("--strict", action="store_true") # Fail if a validation file has no matching original benchmark file.
...
def construct_output_payload(original_payload: dict[str, Any], validation_payload: dict[str, Any], original_path: Path):
...
def main():
    ...
    validation_files = json_helpers.iter_validation_files(validation_root)
    ...
    for validation_path in validation_files:
        ...
        original_payload = json_helpers.load_json(original_path)
        validation_payload = json_helpers.load_json(validation_path)
        output_payload = construct_output_payload(original_payload=original_payload, validation_payload=validation_payload, original_path=original_path)
        ...
        total_kept += kept
        written += 1
        print(f"{validation_path} -> {output_path}: kept {kept} validated-ok sample(s)")
